...
class ITHelpdeskConfig(AgentConfig):
    ...
    def __init__(self):
        # Set the base URL for IT tools server (now internal FastAPI endpoints)
        self.base_url = os.getenv("IT_TOOLS_BASE_URL", "http://localhost:8080")
        ...
        async def lookup_employee_handler(args: Dict[str, Any]) -> Dict[str, Any]:
            ...
            # Get base URL at runtime to ensure environment variable is read correctly
            base_url = os.getenv("IT_TOOLS_BASE_URL", "http://localhost:8080")
            logger.info(f"Using base URL: {base_url}")
            
            try:
                async with aiohttp.ClientSession() as session:
                    url = f"{base_url}/api/v1/test/employees/{employee_id}"
        ...
        async def verify_security_answer_handler(args: Dict[str, Any]) -> Dict[str, Any]:
            ...
            try:
                async with aiohttp.ClientSession() as session:
                    url = f"{base_url}/api/v1/test/verify_security_answer"
                    payload = {
        ...
        async def account_recovery_handler(args: Dict[str, Any]) -> Dict[str, Any]:
            ...
            try:
                async with aiohttp.ClientSession() as session:
                    url = f"{base_url}/api/v1/test/account_recovery"
